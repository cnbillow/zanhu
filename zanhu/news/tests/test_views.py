...
class NewsViewsTest(TestCase):
    ...
    def test_post_news(self):
        ...
        response = self.client.post(
            reverse("news:post_news"), {"post": "这是测试内容"},
            HTTP_X_REQUESTED_WITH='XMLHttpRequest')  # HTTP_X_REQUESTED_WITH='XMLHttpRequest'表示是Ajax request
        assert response.status_code == 200
        assert News.objects.count() == initial_count + 1
    ...
    def test_like_news(self):
        ...
        assert self.user in self.first_news.get_likers()  # uer01在get_likers()获取的点赞用户中
    ...
    def test_updating_interactions(self):
        ...
        assert fourth_response.json()["comments"] == 2  # 在setUp中user02评论了第一条动态，所以是2
